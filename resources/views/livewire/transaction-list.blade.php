<div class="bg-white overflow-hidden shadow-xl sm:rounded-lg item-center border-t border-gray-200">
    <div class="flex flex-row w-full items-center">
        ...
        <div class="flex justify-end text-center items-center">
            <a href="{{ route('transaction.end', ['vehicle' => $vehicle ?? '']) }}" class="p-2 h-full flex-grow items-center text-center bg-yellow-600 hover:bg-yellow-700 rounded-md text-white font-bold">
                <x-heroicon-o-currency-dollar class="w-10 h-full text-center items-center"/>
                <p>{{  __('tags.end')  }}</p>
            </a>
            <a href="{{ route('transaction.create', ['vehicle' => $vehicle ?? '']) }}" class="p-2 h-full items-center text-center bg-green-600 hover:bg-green-700 rounded-md text-white font-bold">
