            ...
            <div class="max-w-md mx-auto">
                ...
                <div class="flex items-center space-x-5">
                    ...
                    <div class="block pl-2 font-semibold text-xl self-start text-gray-700">
                        <h2 class="leading-relaxed">{{ __('tags.edit')." ".trans_choice('tags.user', 2) }}</h2>
                        <p class="text-sm text-gray-500 font-normal leading-relaxed">{{ __('Crear un usuario para la aplicación') }}</p>
                    </div>
                </div>
                        ...
                        <div class="py-8 text-base leading-6 space-y-4 text-gray-700 sm:text-lg sm:leading-7">
                            ...
                            <div class="flex flex-col">
                                ...
                                <input type="text" name="username"
                                    value="{{ $user->username }}"
                                    class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                                    placeholder="{{ __('tags.username') }}">
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">{{ __('tags.password') }}</label>
                                <input type="password" name="password"
                                    class="px-4 py-2 border focus:ring-gray-500 focus:border-gray-900 w-full sm:text-sm border-gray-300 rounded-md focus:outline-none text-gray-600"
                                    placeholder="{{ __('tags.password') }}">
                            </div>
                            <div class="flex flex-col">
                                <label class="leading-loose">{{ __('tags.confirm-password') }}</label>
                            ...
                            <div class="flex flex-col">
                                ...
                                <select name="roles[]" id="roles" class="bg-white text-gray-900 appearance-none border-none inline-block py-3 pl-3 pr-8 rounded leading-tight w-full" multiple>
                                    @foreach($roles as $role)
                                        <option value="{{ $role->id }}" {{ in_array($role->id, $userRole) ? 'selected' : '' }} class="pt-6">{{ $role->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
